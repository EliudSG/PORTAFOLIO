class AudioRecorder {
    ...
    setupListeners() {
        document.addEventListener('DOMContentLoaded',() => { //espera a que el DOM esté cargado (DOM es la estruct de la pagina)
            this.mic.addEventListener('click', () =>this.handleMicClick()); //cuando se hace click en el micro, llama a la función handleMicClick
        });
    }
    async startRecording() { //inicia la grabacion
    ...
    handleMicClick() {
        if (this.estado === 0) {
            ...
            this.stopRecording(); //detiene la grabacion
